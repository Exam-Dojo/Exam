<template>
  <div class="recursion-page">
    ...
    <div class="question-image">
      <img src="@/assets/recursion/recursion10.png" alt="問10の問題文画像" />
    </div>
    ...
    <div class="explanation" v-if="selected" ref="explanation">
      <div class="question-image">
        <img src="@/assets/recursion/recursion10answer.png" alt="問10の解説文画像" />
      </div>
    ...
    <div class="navigation">
      <router-link to="/workbook/recursion9">
        <button class="nav-button">◀ 前の問題</button>
      </router-link>
      <button class="nav-button" @click="confirmAndFinish">終了 ▶</button>
    </div>
  </div>
</template>
...
export default {
  name: 'RecursionPage10',
  components: { StatusPopup },
  props: ['statusMap'],
  data() {
    return {
      selected: null,
      correctAnswer: 'イ', // ★ 正答に応じて変更
      isCorrect: null,
  ...
  methods: {
    ...
    confirmAndFinish() {
      const ok = confirm('本当に終了しますか？終了すると解答状況がリセットされます。')
      if (ok) {
        sessionStorage.removeItem('statusMap')
